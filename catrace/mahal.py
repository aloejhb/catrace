...
def compute_distances_df(
    ...
    if do_shuffle_manifold_pair_labels:
        if shuffle_seed_value is not None:
            ...
            seed_values = master_rng.integers(
                0, 1e9, size=num_odors * num_odors
            ).tolist()
        else:
    ...
    for odor1 in odor_list:
        for odor2 in odor_list:
            ...
            if metric == 'mahal':
                inv_cov_mat2 = invert_cov_mat(
                    np.cov(manifold2.transpose()), reg=reg
                )
                distances = compute_mahals(
                    manifold1.to_numpy(), center2.to_numpy(), inv_cov_mat2
                )
            elif metric == 'euclidean':
                distances = compute_euclideans(
                    manifold1.to_numpy(), center2.to_numpy()
                )
            else:
    ...
    multi_index = pd.MultiIndex.from_tuples(
        distances_dict.keys(), names=['odor', 'ref_odor']
    )

    # Convert to DataFrame using MultiIndex
    distances_df = pd.DataFrame(
        list(distances_dict.values()), index=multi_index
    )
    return distances_df
...
def compute_center_euclidean_distance_mat(
    ...
    if do_shuffle_manifold_pair_labels:
        if shuffle_seed_value is not None:
            master_rng = np.random.default_rng(shuffle_seed_value)
            seed_values = master_rng.integers(
                0, 1e9, size=len(odor_list) * len(odor_list)
            ).tolist()
        else:
...
def get_mean_dist_mat(dist_df, odor_list):
    ...
    dist_mat = dist_mean.pivot_table(
        index='odor', columns='ref_odor', values='mean_dist'
    )

    dist_mat.index = pd.Categorical(
        dist_mat.index, categories=odor_list, ordered=True
    )
...
def plot_mean_dist_mat(dist_mat, ax=None, vmin=None, vmax=None):
    ax = sns.heatmap(
        ...
        vmin=vmin,
        vmax=vmax,
        xticklabels=dist_mat.columns,
        yticklabels=dist_mat.index,
    )
    return ax
