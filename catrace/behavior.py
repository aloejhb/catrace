...
def compute_area_under_the_curve(
    params, param_idx, training_day, num_trials_per_day, num_baseline_days=None
):
    """
    ...
    cs_minus_data = params[
        0, param_idx, :
    ]  # First index (0) corresponds to cs_minus
    cs_plus_data = params[
        1, param_idx, :
    ]  # Second index (1) corresponds to cs_plus
    ...
    if num_baseline_days is not None and num_baseline_days > 0:
        ...
        cs_minus_adjusted = adjust_data_with_baseline(
            cs_minus_data, num_trials_per_day, num_baseline_days
        )
        cs_plus_adjusted = adjust_data_with_baseline(
            cs_plus_data, num_trials_per_day, num_baseline_days
        )

        # Check if there are data points to compute AUC
        if cs_minus_adjusted.size == 0 or cs_plus_adjusted.size == 0:
            raise ValueError(
                'No data points available after baseline adjustment for AUC'
                ' computation.'
            )
    else:
...
def compute_auc_for_all_parameters(
    params, training_day, num_trials_per_day, num_baseline_days=None
):
    """
    ...
    for param_idx in range(num_params):
        auc = compute_area_under_the_curve(
            params,
            param_idx,
            training_day,
            num_trials_per_day,
            num_baseline_days=num_baseline_days,
        )
        aucs.append(auc)
    return aucs
...
def prepare_juvenile_behavior_df(num_baseline_days=None):
    ...
    param_names = [
        str(param_name[0]) for param_name in mat['paramNames'][0]
    ...
    all_behavior_data = mat[
        'allData'
    ]  # Shape: (num_cs, num_params, num_trials, num_fish)

    aucs_list = []

    # Loop over each fish to compute AUCs
    num_fish = all_behavior_data.shape[
        3
    ]  # Fourth dimension corresponds to fish
    for fish_idx in range(num_fish):
        fish_id = fish_ids[fish_idx]
        params = all_behavior_data[
            :, :, :, fish_idx
        ]  # Extract params for this fish
        training_day = training_days[fish_idx]

        # Compute AUC for all parameters for this fish
        aucs = compute_auc_for_all_parameters(
            params, training_day, num_trials_per_day, num_baseline_days
        )
        aucs_list.append((fish_id, training_day, aucs))

    # Create a DataFrame with fish_id, training_day, and aucs
    behavior_df = pd.DataFrame(
        aucs_list, columns=['fish_id', 'training_day', 'aucs']
    )
    ...
    for param_name in param_names:
        auc_columns.extend(
            [f'auc_{param_name}_cs_minus', f'auc_{param_name}_cs_plus']
        )
    ...
    behavior_df = pd.concat(
        [behavior_df.drop(columns=['aucs']), aucs_df], axis=1
    )

    return behavior_df, param_names


def compute_behavior_measures_per_day(
    behavior_df, param_names, num_baseline_days=None
):
    """
    ...
    for param_name in param_names:
        ...
        behavior_measure_df[f'auc_{param_name}_diff'] = (
            behavior_measure_df[f'auc_{param_name}_cs_plus']
            - behavior_measure_df[f'auc_{param_name}_cs_minus']
        )
...
def prepare_juvenile_behavior_timecourse_df(behavior_dir):
    ...
    for cs_idx, cs in enumerate(css):
        for param_idx, param_name in enumerate(param_names):
            ...
            timecourse_df = pd.DataFrame(
                timecourse_mat.T, columns=trials, index=fish_ids
            )
    ...
    behavior_timecourse_df = pd.concat(
        timecourse_dfs, keys=my_keys, names=['cs', 'behavior_param']
    )
