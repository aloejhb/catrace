...
def calc_decorrelation(
    ...
    samecorr = [
        get_same_odor_avgcorr(corrmat_tvec, od, n_trial, sigma)
        for od in odor_range
    ]
    # samecorr = np.stack(samecorr)

    diffcorr = [
        get_paired_odor_avgcorr(corrmat_tvec, odp, n_trial, sigma)
...
def calc_cross_odor_group_corr(corrmat_tvec, aa_range, bb_range, n_trial):
    corr = [
        get_paired_odor_avgcorr(corrmat_tvec, odp, n_trial)
...
def plot_decorrelation(
    ...
    corrmat_tvec = calc_correlation_tvec(
        gaussian_filter1d(trace, sigma0, axis=2)
    )
    cc_corr_avg, cc_corr_std = calc_cross_odor_group_corr(
        corrmat_tvec, aa_range, bb_range, num_trial
    )
    ...
    calc_decorrelation(
        corrmat_tvec, aa_range, ax[0], frame_rate, aa_colors, aa_labels
    )
    plot_avg_std(
        xvec,
        cc_corr_avg,
    ...
    calc_decorrelation(
        corrmat_tvec, bb_range, ax[1], frame_rate, bb_colors, bb_labels
    )
    plot_avg_std(
        xvec,
        cc_corr_avg,
...
def plot_pattern_correlation(
    df, ax=None, clim=None, title='', cmap='turbo', show_legend=False
):
    """
