...
from .process_time_trace import (
    select_odors_and_sort,
...
def normalize_responses(resp_df):
    ...
    resp_df['normalized_response'] = (
        (resp_df.response - baseline) / baseline * 100
    )
    return resp_df
...
def run_response(params: RunResponseParams):
    ...
    resp = compute_response_flattened(
        params.dff, params.time_window, odors, params.top_ratio
    )
    ...
    fig_box, ax, test_results = plot_measure_by_cond(
        resp, 'response', params=params.boxplot_params
    )
...
def run_response_multi_category(params):
    ...
    output_figs = {}
    fig_box, ax, test_results = plot_measure_multi_odor_cond(
        resp,
        'response',
        odor_name='odor_key',
    ...
    for odor_key in params.odor_keys:
        ...
        fig_hist, ax = plot_hist_by_cond(
            sub_resp, 'response', **params.histplot_params
        )
