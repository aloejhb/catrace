...
def compute_mftma(dff, with_center_corr=False, kappa=0, n_t=200, n_reps=1):
    ...
    if with_center_corr:
        alpha_m, radius_m, dimension_m, res_coeff0, KK = manifold_analysis_corr(
            manifolds, kappa=kappa, n_t=n_t, n_reps=n_reps
        )
        ma_result = {
            'alpha_m': alpha_m,
            'radius_m': radius_m,
        ...
        alpha_m, radius_m, dimension_m = manifold_analysis(
            manifolds, kappa, n_t
        )
        ma_result = {
            'alpha_m': alpha_m,
...
def compute_ma_io(
    ...
    **kwargs,
    ...
    if sample_size is not None:
        dff = sample_neuron(
            dff, sample_size=sample_size, random_state=random_state
        )
