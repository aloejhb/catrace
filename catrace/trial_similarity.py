...
def compute_trial_similarity_over_time(
    trial_traces, bin_size=None, similarity_method='pattern_correlation'
):
    # Bin the trial traces if bin_size is specified
    if bin_size is not None:
        trial_traces = trial_traces.groupby(
            trial_traces.index // bin_size
        ).mean()
    ...
    if similarity_method == 'cosine':
        ...
        raise ValueError(
            "Invalid similarity_method. Choose 'cosine' or"
            " 'pattern_correlation'."
        )
...
def plot_correlation_over_time_subplots(
    xvec,
    trial_traces_mean,
    mat,
    ...
    fig, (ax_line, ax_mat) = plt.subplots(
        2,
        1,
        figsize=figsize,
        sharex=True,
...
def compute_trial_similarity_over_time_to_template(
    trial_traces,
    bin_size=None,
    similarity_method='pattern_correlation',
    template=None,
    template_frame_range=None,
):
    # Bin the trial traces if bin_size is specified
    if bin_size is not None:
        trial_traces = trial_traces.groupby(
    ...
    idx = (times >= template_frame_range[0]) & (
        times <= template_frame_range[1]
    )
    template = trial_traces.loc[idx, :].mean()

    # Compute the similarity matrix based on the chosen method
    if similarity_method == 'cosine':
        trial_similarity = cosine_distance_to_template(
            trial_traces.to_numpy(), template
        )
    elif similarity_method == 'pattern_correlation':
        trial_similarity = pattern_correlation_to_template(
            trial_traces.to_numpy(), template
        )
    else:
        raise ValueError(
...
def compute_fish_similarity_over_time(
    ...
    time_idx = trial_traces.index
    # Convert trial_similarity to a DataFrame with the row and column index to be the time index
    trial_similarity = pd.DataFrame(
        trial_similarity, index=time_idx, columns=time_idx
    )
    return trial_similarity
...
def compute_population_trial_similarity_over_time(
    ...
    simdf = pd.concat(
        trial_similarities, keys=exp_list, names=['fish_id', 'condition']
    )
    return simdf
...
def compute_population_multitrial_similarity_over_time(
    ...
    multi_trial_simdf = pd.concat(
        simdfs, keys=odor_trials, names=['odor', 'trial']
    )
...
def plot_matrix_per_condition(
    ...
    conditions,
    frame_range=None,
    params: PlotTrialSimilarityPerCondParams = PlotTrialSimilarityPerCondParams(),
):
    ...
    avg_mats = {
        cond: mat.groupby('time').mean() for cond, mat in avg_mats.items()
    }
...
def compute_population_trial_similarity_over_time_to_template(
    ...
    simtempdf = pd.concat(
        sim_to_templates, keys=exp_list, names=['fish_id', 'condition']
    )
    return simtempdf
...
def compute_population_multitrial_similarity_over_time_to_template(
    ...
    multiodor_simtempdfs = pd.concat(
        simtempdfs, keys=odor_trials, names=['odor', 'trial']
    )
...
def plot_trial_similarity_slicing(
    simdf_slicing,
    metric,
    ax=None,
    color='blue',
    ...
    **kwargs,
    ...
    plot_mean_with_std(
        ...
        start_time_with_zero=start_time_with_zero,
        **kwargs,
    )
...
def plot_slicing_per_condition(
    simtempdf,
    metric,
    colors,
    frame_range=None,
    ...
    **kwargs,
    ...
    for condition, group in simtempdf.groupby('condition'):
        plot_trial_similarity_slicing(
            group.T,
            metric,
            ax=ax,
            color=colors[condition],
            line_label=condition,
