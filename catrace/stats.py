...
def apply_mann_whitney(df, naive_name='naive', trained_name='trained'):
    ...
    for col in df.columns:
        ...
        stat, p = mannwhitneyu(
            naive_data, trained_data, alternative='two-sided'
        )
...
def apply_test_pair(
    ...
    if test_type == 'ttest':
        ...
        p_value = np.mean(
            np.abs(bootstrap_distribution) >= np.abs(observed_diff)
        )

        stat = observed_diff
    else:
        raise ValueError(
            "Invalid test_type. Choose 'ttest', 'mannwhitneyu', or 'bootstrap'"
        )
...
def format_test_results_pair(test_results, test_type='mannwhitneyu'):
    ...
    test_stat_symbols = {'ttest': 't', 'mannwhitneyu': 'U', 'bootstrap': 'Δ'}

    # Since test_results should have only one key-value pair
    if len(test_results) != 1:
        raise ValueError(
            'test_results should contain exactly one comparison result.'
        )
    ...
    for group_pair, result in test_results.items():
        ...
        sentence = (
            f'Comparing {group_name1} (mean = {mean1_formatted}, n = {n1}) vs'
            f' {group_name2} (mean = {mean2_formatted}, n = {n2}):'
            f' {test_names.get(test_type, "Statistical test")},'
            f' {test_stat_symbols.get(test_type, "stat")} ='
            f' {statistic_formatted}, {p_value_formatted}.'
        )
        return sentence
...
def apply_test_each_odor_by_cond(df, yname):
    ...
    for odor_id in df.columns:
        statdf = df[
            [odor_id]
        ].reset_index()  # Reset index to access 'cond' and 'fish_id' as regular columns
        statdf.rename(columns={odor_id: yname}, inplace=True)
        # statdf = statdf.reset_index(level='cond')

        # Kruskal-Wallis test
        data_by_condition = [
            group.values for name, group in statdf.groupby('cond')
        ]
        ...
        if 'naive' in statdf['cond'].unique():
            dunn_test_results = posthoc_dunn(
                statdf, val_col=yname, group_col='cond', p_adjust='bonferroni'
            )
...
def apply_test_by_cond(
    ...
    data_by_condition = [
        group[yname].values
        for name, group in statdf.groupby(cond_name, sort=False, observed=True)
    ]
    ...
    test_results = {
        'Kruskal': {
            ...
            'p_value': p_value,
            'n': {
                cond: summary_stats['count'][cond]
                for cond in summary_stats['count']
            },
        },
...
def pool_training_conditions(df, cond_mapping, keep_subconditions=False):
    ...
    if keep_subconditions:
        df_pooled['subcondition'] = df_pooled.index.get_level_values(
            'condition'
        )
...
def sort_conditions(df, conditions):
    ...
    df.index = df.index.set_levels(
        df.index.levels[idx].astype(cats), level='condition'
    )
...
def format_test_results_by_cond(test_results, naive_name='naive'):
    ...
    P_value_formatted = format_p_value(P_value)
    kruskal_sentence = (
        f'(Kruskal–Wallis test, n = {total_n}, d.f. = {degrees_of_freedom}, H ='
        f' {H_formatted}, {P_value_formatted}).'
    )
    ...
    for cond in n_per_condition.keys():
        ...
        stats_sentence = (
            f'{cond}: mean = {format_number(mean)} ± {format_number(std)} (n ='
            f' {n})'
        )
    ...
    if return_all_pairs:
        dunn_sentence = (
            'Nonparametric multiple comparisons between all groups:'
            f' {comparisons_text}.'
        )
    else:
        n_naive = n_per_condition[naive_name]
        dunn_sentence = (
            f'Nonparametric multiple comparisons against {naive_name} (n ='
            f' {n_naive}): {comparisons_text}.'
        )
...
def format_comparisons(
    ...
    if return_all_pairs:
        ...
        for group_A, group_B in p_values.keys():
            ...
            comparison_text = (
                f'{group_A} (n = {n1}) vs {group_B} (n = {n2}), Q ='
                f' {Q_stat_formatted}, {p_value_formatted}'
            )
        ...
        for cond in p_values.keys():
            ...
            comparison_text = (
                f'{cond}, Q = {Q_stat_formatted}, {p_value_formatted}, n ='
                f' {n_cond}'
            )
            comparisons.append(comparison_text)
...
def apply_tests_multi_odor_two_cond(
    sub_mean_madff, yname, odor_name, test_type='mannwhitneyu'
):
    """
    ...
    for odor, subdf in sub_mean_madff.groupby(odor_name):
        test_results[odor] = apply_test_pair(
            subdf, yname=yname, test_type=test_type
        )
    return test_results


def format_test_results_multi_odor_two_cond(
    test_results, test_type='mannwhitneyu'
):
    """
