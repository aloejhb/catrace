...
def plot_stat_group_vs_group(
    ...
    fig1, axs1, test_results_list1 = plot_all_measures(
        dff,
        measure_names=measure_names,
        name_to_label=None,
        test_type=test_type,
        do_plot_strip=do_plot_strip,
    )
    ...
    fig2, ax2, test_results_list2 = plot_measure(
        dff_shuffled,
        measure_name='capacity',
        name_to_label=None,
        test_type=test_type,
...
def plot_stat_group_vs_group_single_measure(
    dff,
    dff_shuffled,
    measure_name,
    conditions,
    ...
    if do_plot_shuffled:
        plot_measure_params = replace(
            plot_measure_params, do_capitalize_labels=False
        )
    fig, test_results_raw = plot_measure(
        dff,
    ...
    if do_plot_shuffled:
        plot_measure_params = replace(
            plot_measure_params, do_capitalize_labels=do_capitalize_labels
        )
    ...
    if do_plot_shuffled:
        ...
        fig, test_results_shuffled = plot_measure(
            dff_shuffled,
            measure_name=measure_name,
            name_to_label=None,
            test_type=test_type,
...
def plot_adult_and_juv(
    ...
    fig2, ax2, juvenile_test = plot_juv(
        measure_name,
        do_plot_shuffled=do_plot_shuffled,
    ...
    fig1, ax1, adult_test = plot_stat_group_vs_group_single_measure(
        adult_df_per_fish,
        adult_df_per_fish_shuffled,
        measure_name,
        adult_conditions,
...
def plot_juv(
    ...
    fig, ax, test_results = plot_stat_group_vs_group_single_measure(
        df_pooled,
        df_pooled_shuffled,
        measure_name,
        conditions,
...
def plot_cap_and_save(
    ...
    if capacity_ylevel_scale != 1.0:
        plot_measure_params = replace(
            plot_measure_params, ylevel_scale=capacity_ylevel_scale
        )
    fig, ax, test_results = plot_juv(
        odor1_group,
        odor2_group,
        measure_name,
        df_pooled,
    ...
    for idx, measure_name in enumerate(measure_names):
        ...
        if minor_tick_intervals is not None:
            ...
            if minor_tick_interval is not None:
                ax.yaxis.set_minor_locator(
                    plt.MultipleLocator(minor_tick_interval)
                )
        ...
        if ylabel == 'Axes_alignment' or ylabel == 'axes_alignment' or ylabel == 'Axis_alignment' or ylabel == 'axis_alignment':
            ...
            ylabel == 'Center_axes_alignment'
            or ylabel == 'center_axes_alignment'
        ):
...
def load_juvenile_capacities(jobname, analysis_dir):
    ...
    df_per_fish_juv = (
        df_result.xs(False, level='shuffle')
        .groupby(['fish_id', 'condition', 'odor1', 'odor2'], sort=False)
        .mean()
    )
    df_per_fish_juv_shuffled = (
        df_result.xs(True, level='shuffle')
        .groupby(['fish_id', 'condition', 'odor1', 'odor2'], sort=False)
        .mean()
    )
    ...
    df_pooled = pool_training_conditions(
        df_per_fish_juv, cond_mapping, keep_subconditions=True
    )

    df_pooled_shuffled = pool_training_conditions(
        df_per_fish_juv_shuffled, cond_mapping, keep_subconditions=True
    )
    return (
        df_per_fish_juv,
        df_per_fish_juv_shuffled,
