...
def append_neuron_id(dff):
    ...
    if 'plane' in dff.columns.names:
        dff.columns = pd.MultiIndex.from_arrays(
            [
                dff.columns.get_level_values('plane'),
        ...
        dff.columns = pd.MultiIndex.from_arrays(
            [dff.columns.get_level_values('neuron'), neuron_id],
            names=['neuron', 'neuron_id'],
        )
    else:
        # Append a new level called neuron_id to the columns
        dff.columns = pd.MultiIndex.from_arrays(
            [dff.columns, neuron_id], names=[dff.columns.name, 'neuron_id']
        )
    return dff
...
def get_assembly_positions(df, method='top_indices', **kwargs):
    ...
    if method == 'top_indices':
        ...
        args = dict(
            threshold=kwargs['threshold'], percentile=kwargs['percentile']
        )
...
def select_neuron_by_assembly(dff, window, method='top_indices', **kwargs):
    ...
    assembly_matrix = pd.DataFrame(
        False, index=all_neuron_ids, columns=assembly_dict.keys(), dtype=bool
    )
    ...
    dff_select = dff.loc[
        :, dff.columns.get_level_values('neuron_id').isin(all_neuron_ids)
    ]
...
def select_cell_type_odors_neurons(
    dff, cell_type, odors, select_func_name, **kwargs
):
    ...
    if select_func_name == 'select_neuron_by_assembly':
        ...
        raise ValueError(
            f'Unrecognized select_func_name {select_func_name}. So far only'
            ' select_neuron_by_assembly is supported.'
        )
    return dff
    ...
    def __post_init__(self):
        if self.method == 'top_indices':
            if self.assembly_size is None:
                raise ValueError(
                    'assembly_size must be provided when method is'
                    " 'top_indices'."
                )
            if self.threshold is not None or self.percentile is not None:
                raise ValueError(
                    'threshold and percentile must be None when method is'
                    " 'top_indices'."
                )
        elif self.method == 'perc':
            if self.assembly_size is not None:
                raise ValueError(
                    "assembly_size must be None when method is 'perc'."
                )
            if self.threshold is None or self.percentile is None:
                raise ValueError(
                    'threshold and percentile must be provided when method is'
                    " 'perc'."
                )
        else:
            raise ValueError(
                f"Invalid method '{self.method}'. Supported methods are"
                " 'top_indices' and 'perc'."
            )
