...
def compute_noise_levels(trace_df, sampling_rate, baseline_window=None):
    if baseline_window is not None:
        time_indices = trace_df.index.get_level_values('time')
        baseline = trace_df[
            (time_indices >= baseline_window[0])
    ...
    for (odor, trial), subset in baseline.groupby(level=('odor', 'trial')):
        ...
        nl_df = pd.DataFrame(
            noise_levels.reshape(1, -1), columns=subset.columns
        )
        nl_df.index = pd.MultiIndex.from_tuples(
            [(odor, trial)], names=['odor', 'trial']
        )
        nl_dfs.append(nl_df)
...
def deconvolve_experiment(
    ...
    noise_levels = compute_noise_levels(
        trace_df, sampling_rate, baseline_window
    )
    ...
    for (odor, trial), subset in trace_df.groupby(level=('odor', 'trial')):
        ...
        sp_df = pd.DataFrame(
            spike_prob.T, index=subset.index, columns=subset.columns
        )
        sp_dfs.append(sp_df)
