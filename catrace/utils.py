...
def copy_frame_structure(arr, df):
    ...
    assert (
        df.shape == arr.shape
    ), 'Dataframe and array shapes should be the same'
...
def load_json_to_dataclass(filename: str, dataclass_type: DataClassJsonMixin):
    ...
    try:
        ...
        if not hasattr(dataclass_type, 'from_json'):
            raise TypeError(
                f'{dataclass_type.__name__} must be decorated with'
                ' @dataclass_json'
            )
        ...
        raise json.JSONDecodeError(
            f'Invalid JSON in {filename}: {str(e)}', e.doc, e.pos
        )
...
def get_odor_pairs(odor_group1, odor_group2):
    ...
    odor_pairs = [
        (odor1, odor2)
        for odor1 in odor_group1
        for odor2 in odor_group2
        if odor1 != odor2
    ]
    return odor_pairs
...
def get_seed_from_hash(value):
    if value is None:
        ...
        seed = int(hashlib.sha256(str(value).encode()).hexdigest(), 16) % (
            2**32
        )
    return seed
