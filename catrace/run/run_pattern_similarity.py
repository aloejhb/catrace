...
import numpy as np
import pandas as pd
import importlib.pyplot as plt
...
def compute_similarity(params: ComputeSimilarityParams):
    ...
    if not os.path.exists(out_dir) or params.overwrite_computation:
        ...
        compute_pcr_explist = process_data_db_decorator(func, exp_list,
                                                        out_dir, in_dir)
...
def plot_matrix_per_cond(simdf_list, exp_cond_list, conditions):
    ...
    fig, axes = catvis.plot_conds_mat(avg_mats, conditions, plot_similarity_mat, clim=(cmin, cmax), cmap='turbo', ncol=2, ylabel_fontsize=12)
    return fig, axes
...
def extract_cross_trial_similarity(simdf_list, exp_list):
    ...
    cross_trial_df = pd.concat(final_df_list, keys=exp_list, names=['fish_id', 'cond'])
    cross_trial_df.index = cross_trial_df.index.droplevel(-1)
    return cross_trial_df


def save_cross_trial_similarity(cross_trial_df, out_dir, region, metric):
    cross_trial_df.to_pickle(pjoin(out_dir, f'{region}_{metric}_cross_trial_similarity.pkl'))
