...
def compute_similarity(params: ComputeSimilarityParams):
    ...
    out_dir = pjoin(
        in_dir, metric, f'window_{time_window[0]}to{time_window[1]}'
    )
    ...
    if not os.path.exists(out_dir) or params.overwrite_computation:
        ...
        compute_pcr_explist = process_data_db_decorator(
            func, params.exp_list, out_dir, in_dir
        )
        compute_pcr_explist(
            odors=params.odors,
...
def plot_matrix_per_fish(simdf_list, exp_cond_list):
    ...
    fig, axes = plot_explist_with_cond(
        simdf_list,
        exp_cond_list,
        plot_similarity_mat,
        clim=(cmin, cmax),
...
def extract_cross_trial_similarity(simdf_list, exp_list):
    ...
    cross_trial_df = pd.concat(
        final_df_list, keys=exp_list, names=['fish_id', 'condition']
    )
...
def concatenate_simdfs(simdf_lists, exp_list):
    all_simdf = pd.concat(
        simdf_lists, keys=exp_list, names=['fish_id', 'condition']
    )
    return all_simdf
...
def compute_multi_vs(
    vsnames, dsconfig, conditions, all_simdf, metric, naive_name
):
...
def run_pattern_similarity(params: RunPatternSimilarityParams):
    ...
    sim_params = ComputeSimilarityParams(
        in_dir=in_dir,
        exp_list=exp_list,
        metric=params.metric,
        time_window=params.time_window,
