...
def analysis_one_vs_one(params: AnalysisParams):
    ...
    # Preprocessing
    input_manifolds = gcmc.preprocess.standard_preprocessing(
        XtotT,
        global_center=params.global_center,
        bias=params.bias,
        target_num_points_per_manifold=params.M,
    )
    input_manifolds_shuffle = gcmc.preprocess.input_manifolds_shuffle(
        input_manifolds
    )

    # Gaussianize
    if params.gaussianize:
        input_manifolds_gaussianized = gcmc.preprocess.manifold_gaussianization(
            input_manifolds
        )
        input_manifolds_shuffle_gaussianized = (
            gcmc.preprocess.manifold_gaussianization(input_manifolds_shuffle)
        )
        input_manifolds_gaussianized = np.array(input_manifolds_gaussianized)
        input_manifolds_shuffle_gaussianized = np.array(
            input_manifolds_shuffle_gaussianized
        )
    ...
    df_list = []
    matrix_list = {
        'center_alignment_matrix': [],
        'axes_alignment_matrix': [],
        'center_axes_alignment_matrix': [],
    }

    P = input_manifolds.shape[0]
    for i_P in range(P):
        input_manifolds_new = copy.deepcopy(input_manifolds)
        input_manifolds_new[[0, i_P]] = input_manifolds_new[
            [i_P, 0]
        ]  # swap the 0-th manifold and the i_P-th manifold
        result = gcmc.manifold_analysis(
            input_manifolds_new,
            label_sample_type=gcmc.LabelSampleType.FIRST_ELEMENT_ONLY,
        )
        filtered_result = {
            key: value[0]
        ...
        input_manifolds_shuffle_new[[0, i_P]] = input_manifolds_shuffle_new[
            [i_P, 0]
        ]  # swap the 0-th manifold and the i_P-th manifold
        result = gcmc.manifold_analysis(
            input_manifolds_shuffle_new,
            label_sample_type=gcmc.LabelSampleType.FIRST_ELEMENT_ONLY,
        )
        filtered_result = {
            key: value[0]
